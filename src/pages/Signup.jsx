            ...
            <div className="flex items-center gap-2">
              <button type="button" onClick={async () => {
                try {
                  ...
                  const res = await apiPost('/auth/send-otp', { phone: formData.phone, email: formData.email })
                  setOtpSent(true)
                } catch (e) {
                  setErrors(prev => ({ ...prev, phone: e?.error || 'Failed to send OTP' }))
                }
              }} className="text-sm text-blue-600">Send OTP</button>
              {otpVerified && <span className="text-sm text-green-600">Verified</span>}
            </div>

            {otpSent && (
              <div className="flex items-center gap-2">
                <input type="text" value={otpCode} onChange={(e) => setOtpCode(e.target.value)} placeholder="Enter OTP" className="p-2 border rounded" />
                <button type="button" onClick={async () => {
                  try {
                    const res = await apiPost('/auth/verify-otp', { phone: formData.phone, code: otpCode })
                    setOtpVerified(true)
                    if (res && res.verificationToken) setOtpToken(res.verificationToken)
                  } catch (e) {
                    setErrors(prev => ({ ...prev, otp: e?.error || 'OTP verification failed' }))
                  }
                }} className="text-sm bg-blue-600 text-white px-3 py-1 rounded">Verify OTP</button>
              </div>
            )}
