...
import { Headphones, Mail, MessageSquare, ShieldCheck, Sparkles, Timer, Send } from 'lucide-react'
    ...
    <section className="support-page min-h-screen pt-24 pb-16">
      <div className="relative z-10 mx-auto max-w-6xl px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
          ...
          <div className="support-column">
            <div className="support-panel chat-shell border border-gray-100 shadow-sm rounded-[24px] bg-white p-6 relative overflow-hidden flex flex-col h-[650px]">

              <div className="relative z-10 flex items-center justify-between pb-5 border-b border-gray-50 shrink-0">
                <div className="flex items-center gap-3">
                  <div className="h-10 w-10 rounded-xl bg-[#3b82f6] shadow-sm flex items-center justify-center shrink-0">
                    <Sparkles className="h-5 w-5 text-white" />
                  </div>
                  <div>
                    <h2 className="support-font text-lg font-semibold text-gray-900 leading-tight">Dripp Support</h2>
                    <p className="text-xs text-[#3b82f6] font-medium tracking-wide">● Online now</p>
                  </div>
                </div>
                <select
                  className="rounded-lg border border-gray-200 bg-gray-50 text-gray-600 px-3 py-1.5 text-[13px] font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 transition-shadow"
                  value={language}
                  onChange={(event) => setLanguage(event.target.value)}
                >
                  {Object.entries(LANGUAGE_META).map(([key, value]) => (
                    <option key={key} value={key} className="bg-white text-gray-900">{value.label}</option>
                  ))}
                </select>
              </div>

              <div className="relative z-10 pt-4 -mx-2 px-2 flex-1 scrollbar-dark min-h-0">
              ...
              <div className="relative z-10 mt-5 flex flex-wrap gap-2">
                {visibleQuickReplies.map((item) => (
                  <button
                    key={item}
                    type="button"
                    className="quick-chip-dark text-[13px] px-3.5 py-2 rounded-full font-medium text-left"
              ...
              <div className="relative z-10 mt-6 pt-4 border-t border-gray-50 flex items-center gap-2 shrink-0">
                <input
                  type="text"
                  className="flex-1 bg-transparent border-none px-2 py-2 text-[15px] focus:outline-none placeholder:text-gray-400 text-gray-800"
                ...
                <div className="flex items-center gap-2">
                  <button
                    className="text-[13px] font-medium text-gray-500 hover:text-gray-800 transition-colors px-3 py-1.5 rounded-md hover:bg-gray-100 flex items-center justify-center shrink-0"
                    type="button"
                  ...
                  <button
                    className="flex items-center justify-center h-[42px] w-[42px] rounded-full bg-[#1e293b] text-white hover:bg-black transition-colors disabled:opacity-50 shrink-0"
                    type="button"
