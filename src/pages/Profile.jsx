...
export default function Profile() {
  ...
  const savePassword = async () => {
    if ((user.isPasswordSet && !passwords.currentPassword) || !passwords.newPassword) {
    ...
    try {
      await apiPut('/auth/change-password', {
        currentPassword: user.isPasswordSet ? passwords.currentPassword : undefined,
        newPassword: passwords.newPassword
      })
      setEditingPassword(false)
      setPasswords({ currentPassword: '', newPassword: '', confirm: '' })
      setPasswordModal({ open: true, type: 'success', message: 'Password changed successfully.' })
      await fetchMe() // refresh to get updated isPasswordSet status
    } catch (e) {
                  ...
                  <ul className="space-y-2">
                    ...
                    <li>
                      <Link to="/wishlist" className="menu-item block flex justify-between items-center">
                        ...
                        <span className="ml-2 inline-flex items-center justify-center px-2 py-0.5 text-xs font-medium rounded-full bg-gray-100 text-gray-800">{wishlistCount || 0}</span>
                      </Link>
                    </li>
                    <li>
                      <Link to="/orders" className="menu-item block flex justify-between items-center">
                        <span>🧾 My Orders</span>
                        <span className="ml-2 inline-flex items-center justify-center px-2 py-0.5 text-xs font-medium rounded-full bg-gray-100 text-gray-800">{ordersCount || 0}</span>
                      </Link>
                    </li>
                    <li>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div>
                        {editingPhone ? (
                          ...
                          <button className="menu-item menu-item-highlight" onClick={() => setEditingPhone(true)}>{user.phone ? 'Change' : 'Add Phone No'}</button>
                        )}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <div>
                      {editingPassword ? (
                        ...
                        <div>
                          <button className="menu-item menu-item-highlight" onClick={() => setEditingPassword(true)}>
                ...
                <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div className="bg-white border rounded-xl p-4 shadow-sm">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <Link to="/wishlist" className="text-sm font-medium text-gray-900 hover:underline">View all</Link>
                    </div>
                  ...
                  <div className="bg-white border rounded-xl p-4 shadow-sm">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <Link to="/orders" className="text-sm font-medium text-gray-900 hover:underline">View all</Link>
                    </div>
                    {ordersLoading ? (
                        ...
                        {recentOrders.map(o => (
                          <li key={o._id || o.id} className="flex items-center gap-3 p-3 border rounded-md">
                            ...
                            <div className="w-28 flex-shrink-0 text-right">
                              ...
                              <Link to={`/orders/${o._id || o.id}`} className="text-xs font-semibold text-gray-900 hover:underline">View</Link>
                            </div>
                          </li>
                        ))}
                      </ul>
      ...
      {passwordModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-sm w-full p-6 text-center space-y-4">
            <div className={`inline-flex items-center justify-center w-12 h-12 rounded-full ${passwordModal.type === 'success' ? 'bg-gray-100 text-gray-900' : 'bg-red-50 text-red-600'}`}>
            ...
            <button
              className="w-full bg-black text-white h-11 rounded-xl font-semibold hover:bg-gray-800 transition-colors"
